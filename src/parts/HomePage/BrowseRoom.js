...
function Loading({ ratio = {} }) {
  ...
  return dummy.map((item, index) => {
    return (
      ...
      >
        <div className="loading-preview w-full h-full rounded-lg">
          <div className={`overlay ${ratio?.meta?.[item.ratio.md]}`}>
            <div className="bg-gray-400 md:w-28 w-16 h-5 rounded-md mb-3"></div>
            <div className="bg-gray-400 md:w-28 w-12 h-3 rounded-md"></div>
          </div>
        </div>
...
export default function BrowseRoom() {
  ...
  useEffect(() => {
    run(
      fetch(
        "https://2fc1f965-5454-4a3f-9797-87cdf0ca6b9d.mock.pstmn.io/api/categories/?page=1&limit=4"
      ).then(async (response) => {
